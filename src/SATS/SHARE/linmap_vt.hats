...
absvtype
map_vtype (key:vt@ype, itm:vt@ype+) = ptr
...
copy_key (x1: !key): key
fun{key:vt0p}
free_key (x1: key):<!wrt> void

(* ****** ****** *)

fun{}
linmap_nil {key:vt0p;itm:vt0p} ():<> map (key, itm)
fun{}
linmap_make_nil {key:vt0p;itm:vt0p} ():<!wrt> map (key, itm)

(* ****** ****** *)

fun{
} linmap_is_nil
  {key:vt0p;itm:vt0p} (map: !map (key, INV(itm))):<> bool
...
key:vt0p;itm:vt0p
  ...
  (map: !map (key, INV(itm)), k0: !key): P2tr0 (itm)
...
fun{
key:vt0p;itm:vt0p
} linmap_freelin$clear (k: &key >> _?, x: &itm >> _?):<!wrt> void
fun{
key:vt0p;itm:vt0p
